...
public:
    // TODO: use mempool
    CudaMem(int size) {
        SAFE_CALL(cudaMalloc<unsigned char>(&ptr_, size),"CUDA Malloc Failed");
    }
    ~CudaMem() {
        SAFE_CALL(cudaFree(ptr_),"CUDA Free Failed");
    }
...
void downscaleCuda(const cv::Mat& input, cv::Mat& output)
{
    ...
    CudaMem d_input(inputBytes);
    CudaMem d_output(outputBytes);
