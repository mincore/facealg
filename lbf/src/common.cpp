...
BBox::BBox(const FaceInfo& faceInfo) {
...
Mat drawShapeInImage(const Mat &img, const FacePts &shape, const BBox &bbox) {
    Mat img_ = img.clone();
    rectangle(img_, Rect(bbox.x, bbox.y, bbox.width, bbox.height), Scalar(0, 0, 255), 2);
    for (int i = 0; i < 5; i++) {
        circle(img_, Point(shape.x[i], shape.y[i]), 2, Scalar(0, 255, 0), -1);
    }
    return img_;
}
...
Config::Config() {
    dataset = "/mnt/nfs/wenge/data/CelebA/Anno/trainset";
    saved_file_name = "../model/celeba_.model";
    stages_n = 5;
    tree_n = 12;
    tree_depth = 5;
    landmark_n = 5;
