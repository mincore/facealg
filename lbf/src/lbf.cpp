...
Mat LbfCascador::GlobalRegressionPredict(const Mat &lbf, int stage) {
    ...
    //#pragma omp parallel for num_threads(32) private(w_ptr)
...
bool LbfCascador::predict(Mat &img, FaceInfo& faceInfo) {
#ifndef NDEBUG
    double t1 = getTime();
#endif
    Mat gray;
    ...
    for (int k = 0; k < stages_n; k++) {
        // generate lbf
        Mat lbf = random_forests[k].GenerateLBF(gray, current_shape, bbox, mean_shape);
        // update current_shapes
        Mat delta_shape = GlobalRegressionPredict(lbf, k);
        delta_shape = delta_shape.reshape(0, landmark_n);
        current_shape = bbox.Project(current_shape);
        calcSimilarityTransform(current_shape, mean_shape, scale, rotate);
        current_shape = bbox.ReProject(current_shape + scale * delta_shape * rotate.t());
    }
    for (int i = 0; i < 5; i++){
        faceInfo.facePts.x[i] = current_shape.at<double>(i,0);
        faceInfo.facePts.y[i] = current_shape.at<double>(i,1);
    }
#ifndef NDEBUG
    double t2 = getTime();
    std::cout << "Time of lbf: " << t2-t1 << std::endl;
#endif
    return true;
}

void LbfCascador::prealloc(int times) {
    Mat img(Size(100,100), CV_32FC3);
    randu(img, Scalar::all(0), Scalar::all(255));
    FaceInfo faceInfo;
    for (int i = 0; i < times; ++i) {
        predict(img, faceInfo);
    }
}

void LbfCascador::init(NYUnpacker &unp, const char* modelDirPath) {
    std::vector<char> data;
    if (!unp.read(modelDirPath, data)) {
        printf("unpack lbfmodel:%s failed\n", modelDirPath);
        return;
    }
    Config &config = Config::GetInstance();
    // global parameters
#if 0
    FILE *fd = fopen(modelDirPath, "rb");
...
#else
    int *pint = (int*)&data[0];
    config.stages_n = *pint++;
    config.tree_n = *pint++;
    config.tree_depth = *pint++;
    config.landmark_n = *pint++;
#endif
    ...
    // mean_shape
    for (int i = 0; i < mean_shape.rows; i++) {
        double *ptr = mean_shape.ptr<double>(i);
        int size = sizeof(double)*mean_shape.cols;
        memcpy(ptr, pbuf, size);
        pbuf += size;
    }
    // every stages
    for (int k = 0; k < stages_n; k++) {
        pbuf = random_forests[k].Read(pbuf);
        for (int i = 0; i < 2 * config.landmark_n; i++) {
            double *ptr = gl_regression_weights[k].ptr<double>(i);
            int size = sizeof(double) * gl_regression_weights[k].cols;
            memcpy(ptr, pbuf, size);
            pbuf += size;
        }
    }
