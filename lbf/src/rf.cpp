...
void RandomTree::Train(vector<Mat> &imgs, vector<Mat> &current_shapes, vector<BBox> &bboxes, \
    ...
    for (size_t i = 0; i < delta_shapes.size(); i++) {
...
char *RandomTree::Read(char *buf)
{
    ...
    for (int i = 1; i < nodes_n / 2; i++) {
        memcpy(feats.ptr<double>(i), buf, size);
        buf += size;

        thresholds[i] = *(int*)buf;
        buf += sizeof(int);
    }
...
char *RandomForest::Read(char *buf)
{
    Config &config = Config::GetInstance();
    // initialize
