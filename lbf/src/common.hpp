...
std::vector<cv::Mat> getDeltaShapes(std::vector<cv::Mat> &gt_shapes,
        std::vector<cv::Mat> &current_shapes,
        std::vector<BBox> &bboxes,
        cv::Mat &mean_shape);

cv::Mat drawShapeInImage(const cv::Mat &img, const FacePts &shape, const BBox &bbox);
